...
df_grid <- tibble(date = seq(start_day, end_day, by = "1 day")) %>% 
  mutate(year = year(date),
         ...
         # first_day_of_month = floor_date(date, "month"),  # Finding the first day of the month
         # week_of_year = floor((date - first_day_of_month + 1) / 7) + 1) %>% 
         # week_of_year = isoweek(date)) %>% 
         # week_of_year = week(date)) %>% 
         # first_day_of_year = floor_date(date, "year"),  # Finding the first day of the year
         # week_of_year = as.integer((date - first_day_of_year + 3) / 7) + 1) %>%
...
df_labels <- df_grid %>% 
  ...
  slice(1) 
  # filter(num %in% c(1, 37, 65, 93, 121, 156, 184, 219, 247, 275, 310, 338))
...
ggplot() +
  ...
  theme(plot.title = element_text(family = font, size = 22, hjust = 0, color = "#FFFFFF"),
        ...
        plot.subtitle = element_text(family = font, size = 10, hjust = 0, color = "#FFFFFF", margin = margin(t = 5, b = 10)),
  ...
  labs(title = "Running Activity",
       subtitle = "Activity is based on total distance run each day and grouped into five categories: 0 miles, < 6 miles,\n6 to 12 miles, 12 to 20 miles, and > 20 miles",
